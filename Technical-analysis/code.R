...
signal_MACD2=1*(MACD(Cl(spy), nSig = 18)$signal>0)%>%Lag()%>% replace(is.na(.), 0),
dailyreturn=dailyReturn(spy)%>%Lag()%>% replace(is.na(.), 0),
weeklyreturn=SMA(dailyReturn(spy),n=5)%>%Lag()%>% replace(is.na(.), 0))

tkrs=unique(c("ETHQ.TO","CTS.V","RSI.TO","GDNP.V","LSPD.TO","ATE.TO","EDGE.TO","ZEO.TO","ZRE.TO","RCI-B.TO","BTO.TO","ABX.TO","NWH-UN.TO","SJR-B.TO","IHI","XEG.TO","XMA.TO","XIN.TO","XMD.TO","XIT.TO","SU.TO","MFC.TO","TRP.TO","T.TO","BEPC.TO","KL.TO","CGL.TO","APHA.TO","SKYY","RNW.TO","PSI","ICLN","HEO.V","ABT.TO","ARKK","ETHQ.TO","CTS.V","RSI.TO","GDNP.V","LSPD.TO","ATE.TO","IPA.V","LIFE.TO","MCHI","XCH.TO","IHF","ZEO.TO","XGD.TO","ZRE.TO","HACK","RCI-B.TO","BTO.TO","ABX.TO","NWH-UN.TO","XTR.TO","XEG.TO","XMA.TO","XIN.TO","XMD.TO","XIT.TO","BIP-UN.TO","AP-UN.TO","RTH","TAN","PBD","EMQQ","KGRN","VDY.TO","ROBO","BLDP.TO","AC.TO","DGRO","BCE.TO","ATZ.TO","ARRY","ZCH.TO","REI-UN.TO","BB.TO","ABT.TO","GNOM","ARKG","VGRO.TO","SCHD","WCN.TO","RBA.TO","VRE.TO","XDSR.TO","BEP-UN.TO","XEQT.TO","XQQ.TO","VFV.TO","CNR.TO","XUH.TO","XUU.TO","PSI","IGV","PSJ","SOXX","IHI","IVW","XST.TO","XUT.TO","CPX.TO","HXS.TO","HXQ.TO","XHC.TO"))
...
ret <- dailyReturn(spy)*(1*(buy_trd_crt>0))+ dailyReturn(spy)*(1*!(sell_trd_crt>0))
sum(ret)

ret <- dailyReturn(spy)*(1*(buy_trd_crt>0))
sum(ret)

ret <-  dailyReturn(spy)*(1*!(sell_trd_crt>0))
sum(ret)
...
sum(!(dailyReturn(spy)*(1*(buy_trd_crt>0))+ dailyReturn(spy)*(1*!(sell_trd_crt>0))==0))
