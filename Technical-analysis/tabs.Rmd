...
```{r, echo=FALSE}
p=day_tab%>%filter(signal_B!=0)%>%select(signal_B,stock)%>%mutate(stock=as.factor(stock))%>%arrange(desc(signal_B))%>%
  ggplot(aes(x=reorder(stock, signal_B),y=signal_B))+
  scale_fill_gradient(low = "red", high = "green") + 
  geom_bar(position="dodge", stat="identity", aes(fill = signal_B))+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  xlab("Stock")+ ylab("signal_B")+ theme(legend.position = "none")
ggplotly(p)

```

```{r, echo=FALSE}
p=day_tab%>%filter(signal_B!=0)%>%select(weeklyreturn,stock)%>%mutate(stock=as.factor(stock))%>%arrange(desc(weeklyreturn))%>%
